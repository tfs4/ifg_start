     ...
     init: function(req_urls) {
        var _this = this;
         $('#id_grupo').on('change', function(){
             ...
             if(grupo == "A"){
                 alert("teste")
                  $('#id_modalidade_tarifaria').options[2].disabled = true;
             }else{
                 $('#id_modalidade_tarifaria').options[2].disabled = true;
             }

            // var postData = {
            //     'cod_uf': $('#id_modalidade_tarifaria').val(),
            //     'cod_gr': $('#id_grupo').val(),
            //
            // }
            // /$.Admin.ZeppelinajaxRequest.ajaxPostRequest(req_urls['info_emit_url'], postData, _this.handleEexemploAjax);
        });
     },
    handleEexemploAjax: function(data){
        //window.location.replace(data.url);
        // for(var i = 0; i < data.length; i++) {
        //     alert("aqui")
